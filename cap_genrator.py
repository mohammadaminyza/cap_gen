...
for char in list(string.digits):
    for i in range(1000):
        ...
        if not os.path.exists(f"captcha_images_v2/{char}"):  # Check if folder exists, if not, create it
            os.makedirs(f"captcha_images_v2/{char}")
        captcha_image.save(f"captcha_images_v2/{char}/{captcha_text}_{uuid.uuid4()}.png")

        print(f"{i} captcha image saved!")
